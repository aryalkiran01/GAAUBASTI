...
const createNotification = async ({ userId, type, content }) => {
  ...
  const existing = await Notification.findOne({ user: userId, 'content.dedupKey': dedupKey });
...
const sendTransactionalEmail = async (templateKey, to, data) => {
  ...
  try {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.error(`[Email] Failed to send ${templateKey} to ${to}:`, error.message);
    }
    return { sent: false, reason: 'SEND_FAILED' };
  }
};

const sendTransactionalSMS = async (to, data) => {
  if (!to) return { sent: false, reason: 'NO_RECIPIENT' };
  if (!isSMSConfigured()) {
    if (process.env.NODE_ENV === 'development') {
...
const notifyBookingCreated = async ({ booking, guest, host }) => {
  const content = {
  ...
  await createNotification({ userId: String(booking.guest), type: 'booking_created', content });
  if (host) {
    await createNotification({ userId: String(booking.host), type: 'new_booking', content });
  }
...
const notifyPaymentConfirmed = async ({ booking, guest, host, payment }) => {
  const content = {
    bookingId: String(booking._id),
  ...
  await createNotification({ userId: String(booking.guest), type: 'payment_confirmed', content });
  if (host) {
    await createNotification({ userId: String(booking.host), type: 'payment_received', content });
  }
...
const notifyBookingCancelled = async ({ booking, guest, host, refundAmount }) => {
  const content = {
    bookingId: String(booking._id),
    listingTitle: booking.listing?.title || 'Your stay',
    refundAmount,
  ...
  await createNotification({ userId: String(booking.guest), type: 'booking_cancelled', content });
  if (host) {
    await createNotification({ userId: String(booking.host), type: 'booking_cancelled', content });
  }
...
const notifyRefundProcessed = async ({ booking, guest, amount }) => {
  const content = {
    bookingId: String(booking._id),
    amount,
  ...
  await createNotification({ userId: String(booking.guest), type: 'refund_processed', content });
...
const notifyNewMessage = async ({ conversation, sender, recipient }) => {
  const content = {
  ...
  await createNotification({ userId: String(recipient._id), type: 'new_message', content });
...
const notifyPayoutCreated = async ({ payout, host }) => {
  const content = {
  ...
  await createNotification({ userId: String(payout.host), type: 'payout_created', content });
...
const notifyReviewReminder = async ({ booking, guest }) => {
  const content = {
    bookingId: String(booking._id),
  ...
  await createNotification({ userId: String(guest._id), type: 'review_reminder', content });
...
const notifySafetyAlert = async ({ user, message }) => {
  const content = {
    preview: message,
    dedupKey: `safety_alert:${String(user._id)}:${Date.now()}`
  };

  await createNotification({ userId: String(user._id), type: 'safety_alert', content });

  if (user.phone) {
    await sendTransactionalSMS(user.phone, { body: `Gaun Basti Safety Alert: ${message}` });
  }
};
