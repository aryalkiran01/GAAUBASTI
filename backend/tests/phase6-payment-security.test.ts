...
import { ensureBookingIsPayable, verifyPaymentOwnership, normalizeAmount } from '../controllers/paymentController.ts';
...
test('ensureBookingIsPayable rejects mismatched booking owner and amount', async () => {
  ...
  const result = await ensureBookingIsPayable(
    { bookingId: 'booking_1', userId: { toString: () => 'guest_2' }, amount: 120.5, listingId: 'listing_1' },
    mockBooking as any
  );
...
test('ensureBookingIsPayable rejects amount mismatch before payment initialization', async () => {
  ...
  const result = await ensureBookingIsPayable(
    { bookingId: 'booking_2', userId: { toString: () => 'guest_3' }, amount: 100, listingId: 'listing_2' },
    mockBooking as any
  );
...
test('verifyPaymentOwnership blocks forged payer and mismatched provider payment IDs', async () => {
  ...
  const forgedByOtherUser = await verifyPaymentOwnership(
    { paymentId: 'payment_1', userId: { toString: () => 'guest_9' }, providerPaymentId: 'pi_correct', amount: 80 },
    mockPayment as any
  );
  ...
  const providerMismatch = await verifyPaymentOwnership(
    { paymentId: 'payment_1', userId: { toString: () => 'guest_4' }, providerPaymentId: 'pi_wrong', amount: 80 },
    mockPayment as any
  );
